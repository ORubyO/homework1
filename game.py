...
while running:
    ...
    if len(enemies) == 0:
        ...
        movements = ENEMY_MOVEMENT_SET.copy() # 重設 movements (複製)
        pg.time.set_timer(ENEMYMOVE_EVENT, ENEMY_MOVE_TIME_MAX) # 重設敵人移動計時器
    ...
    for event in pg.event.get(): # 取得事件
        ...
        # 敵人射擊 (隨機)
        elif event.type == ENEMYSHOOT_EVENT:
            if random.random() > ENEMY_SHOOT_PROBABILITY: # 取得 [0, 1) 之間的隨機數字，若大於 ENEMY_SHOOT_PROBABILITY 則發射
